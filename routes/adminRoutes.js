...
router.patch("/orders/:orderId/status", async (req, res) => {
  ...
  try {
    ...
    if (previousStatus === orderStatus) {
      // Nothing changed: no update and no duplicate email.
      return res.json({ success: true, order, emailSent: false, unchanged: true });
    }

    order.orderStatus = orderStatus;
    await order.save();

    const mail = await sendTemplate("orderStatusChanged", "customer", order, {
      to: order.shippingDetails?.email || order.userId?.email,
      previousStatus,
    });

    return res.json({ success: true, order, emailSent: Boolean(mail.sent) });
  } catch (error) {
